...
def valid_space(shape, grid):
    accepted_pos = [[(j, i) for j in range(10) if grid[i][j] == (0,0,0)] for i in range(20)] # ex: [[(1, 2)], [(2, 3)]]
    accepted_pos = [j for sub in accepted_pos for j in sub]         # ex: [(1, 2), (2, 3)]
...
def draw_next_shape(shape, surface):
    ...
    sy = top_left_y + play_height/2 - 30
    format = shape.shape[shape.rotation % len(shape.shape)]
